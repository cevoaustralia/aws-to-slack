...
const ToAddresses = _.compact(
	_.map(_.split(process.env.EMAIL_LIST, ","), (a) => _.trim(a)),
);
...
<div style="padding:4px 4px 4px 1em;border-left:4px solid ${color}">`);
	...
	if (att.author_name) {
		const authLink = !att.author_link
			? ""
			: `href="${escape(att.author_link)}"`;
		html.push(
			`<a style="display:block;font-size:1.6em;margin:.1em 0;font-weight:bold" ${authLink}>${escape(att.author_name)}</a>`,
		);
		text.push(
			authLink ? `${att.author_name}\n${att.author_link}\n` : att.author_name,
		);
	}
	if (att.title) {
		const titleLink = !att.title_link ? "" : `href="${escape(att.title_link)}"`;
		html.push(
			`<a style="display:block;font-size:1.3em;color:#999;" ${titleLink}>${escape(att.title)}</a>`,
		);
	...
	if (att.fields && att.fields.length) {
		...
		_.each(att.fields, (field) => {
			html.push(
				`<div style="float:left;width:48%"><div style="font-weight:bold">${escape(field.title)}`,
			);
			html.push(
				`</div><p style="margin:.5em 0">${escape(field.value)}</p></div>`,
			);
	...
	html.push("</div>");
	html.push(
		'<div style="border-top:1px solid #eceef9;color:#999;padding:.5em 0 0;margin:1em 1.5em;font-size:.8em">',
	);
	...
	if (lambdaName) {
		html.push(
			`<p style="margin:.5em 0">Generated by <a href="https://console.aws.amazon.com/lambda/home?region=${region}#/functions/${lambdaName}">Lambda ${lambdaName} in ${region}</a></p>`,
		);
	...
	if (event) {
		html.push(
			'<p style="margin:.5em 0">This message was generated from the following raw event:</p>',
		);
		html.push(
			`<pre style="color:#ccc;color:rgba(0,0,0,.08);font-size:.5em;font-weight:100;line-height:.8;letter-spacing:-1px;word-break:break-all;white-space:pre-wrap;">${escape(JSON.stringify(event))}</pre>`,
		);
	}
	html.push("</div>");
